...
const LimitedLiabilityPartnership = (props: any) => {
    ...
    return (
        <div className="grid md:grid-cols-2 gap-5">
            <div>
                <p className="font-medium text-[16px] mb-3">Registered partnership name</p>
                <div className="relative w-full">
                    <input value={partnershipName} onChange={e=>setPartnershipName(e.target.value)} type="search" id="search-dropdown" className="block p-2 w-full  px-3 text-sm text-gray-900 bg-gray-50 rounded-r-lg rounded-l-lg border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Search..." required />
                    <button type="submit" className="absolute top-0 right-0 p-2 text-sm font-medium text-white rounded-r-lg border border-gray-300 focus:ring-4 focus:outline-none">
                        <svg aria-hidden="true" className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke="black" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                        <span className="sr-only">Search</span>
                    </button>
                </div>
                <p className="text-sm font-thin py-2">Can't find the company? <a className="underline-offset-1">Enter it manually</a></p>
            </div>
            <div>
                ...
                <p className="text-sm font-thin py-2">New business? Enter the expected launch date.</p>
            </div>
            <div>
